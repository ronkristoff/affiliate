  ...
  const columns: TableColumn<Broadcast>[] = [
    {
      ...
      cell: (row) => (
        <Link
          ...
          className="font-medium truncate hover:text-[var(--brand-primary)] transition-colors"
...
// ---------------------------------------------------------------------------
// Broadcast History Page (client component with hooks)
// ---------------------------------------------------------------------------
            ...
            <div className="px-5 py-4">
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setStatusFilter(
                      v as "all" | "sent" | "partial" | "failed" | "sending"
                    )
                  }
                >
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setDateRangeFilter(v as "all" | "7d" | "30d" | "90d")
                  }
                >
                  <SelectTrigger className="w-full sm:w-[160px]">
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="recipientCount-desc">
                      Most recipients
                    </SelectItem>
                    <SelectItem value="recipientCount-asc">
                      Fewest recipients
                    </SelectItem>
                  </SelectContent>
                </Select>
              ...
              <div className="md:hidden divide-y">
                {sortedBroadcasts.length === 0 ? (
                  ...
                  sortedBroadcasts.map((broadcast) => {
                    const statusConfig =
                      STATUS_CONFIG[broadcast.status] || STATUS_CONFIG.pending;
                    return (
                      <Link
                        key={broadcast._id}
                        href={`/emails/history/${broadcast._id}`}
                        className="block hover:bg-[var(--bg-page)] transition-colors"
                      >
                        <div className="px-4 py-4">
                          <div className="flex items-start gap-3">
                            <div className="h-10 w-10 rounded-full bg-[var(--bg-page)] flex items-center justify-center flex-shrink-0 mt-0.5">
              ...
              {broadcasts.page.length > 0 && (
                <div className="flex items-center justify-between px-6 py-4 border-t bg-[var(--bg-page)]">
                  <p className="text-sm text-[var(--text-muted)]">
                    Showing {sortedBroadcasts.length} of{" "}
                    {broadcasts.page.length} broadcasts
                  </p>
                  <div className="flex gap-2">
                    ...
                    {!broadcasts.isDone && broadcasts.continueCursor && (
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          setCursor(broadcasts.continueCursor)
                        }
                      >
                        Load more
                      </Button>
